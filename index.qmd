...
<img src="lpo_logo.jpg" alt="Logo Gauche" style="height: 100px;"/> <img src="reserve_logo.png" alt="Logo Droit" style="height: 100px;"/>
:::
...
::: {.logo-row style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1em;"}
<img src="Logo-FSE-UE-Region-Nouvelle-Aquitaine.png" alt="Logo Gauche" style="height: 100px;"/> <img src="Logo-Prefet-de-la-Region-Nouvelle-Aquitaine.png" alt="Logo Droit" style="height: 100px;"/>
:::
...
Le Courlis cendré est le plus grand limicole d’Europe. Longueur : 55–65 cm. Envergure : 90–110 cm. Poids : 600–1200 g. Plumage brun-gris tacheté, bec long (jusqu’à 19 cm), courbé vers le bas. Dimorphisme sexuel marqué ; la femelle est généralement plus grande avec un bec plus long.
...
Les études sur la zone globale ont été effectuées avec un grain spatial large en grille de 100 m x 100 m. Les études sur les zones réduites A, B, C, D et E ont été effectués avec un grain spatial fin en grille de 10 m x 10 m.
...
Zones d'études - zone d'étude globale (zone blanche) et zones d'études réduites (zones AA, BB, CC)
:::
...
Les reposoirs sont estimés :

-   à partir des données de l'ensemble des individus via la délimitation de zone "hotspot" [Bakker et al. (2021)](#bakker2021) et de façon globale sur toute la zone d'études (sur des grilles de 100m x 100m, @fig-UDMap_roosting_hotspot_from1id)
-   de façon plus fine sur les zones spécifiques A, B, C, D et E (sur des grilles de 10m x 10m, @fig-UDMap_roosting_A, @fig-UDMap_roosting_B, @fig-UDMap_roosting_C, @fig-UDMap_roosting_D, @fig-UDMap_roosting_E).
...
tags$iframe(
  src = "UDMap_roosting_95_50_AA.html",
  width = "100%",
  height = "400px"
)
```
...
::: {#fig-UDMap_roosting_BB}
```{r}
#| echo: false

tags$iframe(
  src = "UDMap_roosting_95_50_BB.html",
  width = "100%",
  height = "400px"
)
```
...
tags$iframe(
  src = "UDMap_roosting_95_50_CC.html",
  width = "100%",
  height = "400px"
)
```
...
::: {#fig-UDMap_roosting_month_A}
```{r}
#| echo: false

tags$iframe(
  src = "UDMap_roosting_month_A.html",
  width = "100%",
  height = "400px"
)
```
...
::: {#fig-UDMap_roosting_month_B}
```{r}
#| echo: false

tags$iframe(
  src = "UDMap_roosting_month_B.html",
  width = "100%",
  height = "400px"
)
```
...
::: {#fig-UDMap_roosting_month_C}
```{r}
#| echo: false

tags$iframe(
  src = "UDMap_roosting_month_C.html",
  width = "100%",
  height = "400px"
)
```
...
::: {#fig-UDMap_roosting_month_D}
```{r}
#| echo: false

tags$iframe(
  src = "UDMap_roosting_month_D.html",
  width = "100%",
  height = "400px"
)
```

Reposoirs zone D en fonction du mois
:::

### Zone E

::: {#fig-UDMap_roosting_month_E}
```{r}
#| echo: false

tags$iframe(
  src = "UDMap_roosting_month_E.html",
  width = "100%",
  height = "400px"
)
```

Reposoirs zone E en fonction du mois
:::
::::::::
...
Les zones d'alimentation sont estimées :
...
::: {#fig-UDMap_foraging_AA}
```{r}
#| echo: false

tags$iframe(
  src = "UDMap_foraging_95_50_AA.html",
  width = "100%",
  height = "400px"
)
```

Alimentation zone AA
:::

### Zone BB

::: {#fig-UDMap_foraging_BB}
```{r}
#| echo: false

tags$iframe(
  src = "UDMap_foraging_95_50_BB.html",
  width = "100%",
  height = "400px"
)
```

Alimentation zone BB
:::

### Zone CC

::: {#fig-UDMap_foraging_CC}
```{r}
#| echo: false

tags$iframe(
  src = "UDMap_foraging_95_50_CC.html",
  width = "100%",
  height = "400px"
)
```

Alimentation zone CC
:::
::::::::
...
::: {#fig-UDMap_foraging_month_A}
```{r}
#| echo: false

tags$iframe(
  src = "UDMap_foraging_month_A.html",
  width = "100%",
  height = "400px"
)
```
...
::: {#fig-UDMap_foraging_month_B}
```{r}
#| echo: false

tags$iframe(
  src = "UDMap_foraging_month_B.html",
  width = "100%",
  height = "400px"
)
```
...
::: {#fig-UDMap_foraging_month_C}
```{r}
#| echo: false

tags$iframe(
  src = "UDMap_foraging_month_C.html",
  width = "100%",
  height = "400px"
)
```
...
::: {#fig-UDMap_foraging_month_D}
```{r}
#| echo: false

tags$iframe(
  src = "UDMap_foraging_month_D.html",
  width = "100%",
  height = "400px"
)
```

Zone d'alimentation zone D en fonction du mois
:::

### Zone E

::: {#fig-UDMap_foraging_month_E}
```{r}
#| echo: false

tags$iframe(
  src = "UDMap_foraging_month_E.html",
  width = "100%",
  height = "400px"
)
```

Zone d'alimentation zone E en fonction du mois
:::
::::::::
...
## Zone critique de repos et d'alimentation

::: {#fig-zone_critique_hotsport_map_v1}
```{r}
#| echo: false

tags$iframe(
  src = "zone_critique_hotsport_map_v1.html",
  width = "100%",
  height = "400px"
)
```
...
La distance entre la zone d'alimentation et de repos a été estimé comme la distance entre les paires de centroïds géographiques individuels des zones d'alimentation et de repos à chaque cycle de marais (@fig-distance_roost_forag_allvar_plot).
...
Les distances à chaque cycle de marais des paires de centroïds individuels entre les zones d'alimentation et de repos ont été comparées entre les mâles et les femelles, les adultes et les juvéniles, et pendant ou hors péridoe de chasse (@fig-distance_roost_forag_age_sex_plot).
...
## Quelles zones de report des oiseaux lors des fluctuations environnementales et des perturbations anthropiques ?
...
Les reposoirs peuvent varier en fonction de la hauteur d'eau lors des marées hautes (@fig-UDMap_roosting_tides_high_type_A, @fig-UDMap_roosting_tides_high_type_B, @fig-UDMap_roosting_tides_high_type_C, @fig-UDMap_roosting_tides_high_type_D, @fig-UDMap_roosting_tides_high_type_E).
...
::: {#fig-UDMap_roosting_chasse_in_out_saison_C}
```{r}
#| echo: false

tags$iframe(
  src = "UDMap_roosting_chasse_in_out_saison_C.html",
  width = "100%",
  height = "400px"
)
```
...
::: {#fig-UDMap_foraging_chasse_in_out_saison_C}
```{r}
#| echo: false

tags$iframe(
  src = "UDMap_foraging_chasse_in_out_saison_C.html",
  width = "100%",
  height = "400px"
)
```
...
Afin d'étudier l'effet de la chasse à la tonnes (point noir) et des tonnes de chasse sur l'utilisation de l'espace des Courlis cendré, deux zones ont été définies (@fig-map_tonnes_v2) :

-   Zone de danger : zone d'un rayon de 300m entourant chaque tonne de chasse (en orange)
-   Zone de proximité : zone d'un rayon de 1.5 km entourant chaque tonne de chasse (en jaune)
...
Pour estimer les effets de la période de chasse (hors ou pendant la période de chasse) et de la zone (de danger ou de proximité) sur la répartition spatiale des points GPS enregistrés pour chaque individu, le modèle linéaire mixte suivant a été appliqué via le packages R "lmerTest":
...
Puisqu'un même individu compte plusieurs points GPS enregistrés, et que ces points ne sont donc pas indépendants entre eux, un effet aléaoire $IND_i$ a été ajouté. $\varepsilon_{it}$ correspond aux résidus du modèle, autrement dit à la variation dans le nombre de points GPS $Nbpoint_{it}$ qui n'est pas expliqué par ces variables.

Puisque la chasse à la tonne se pratique de nuit, uniquement les points GPS enregistrés de nuit ont été inclus dans l'analyse.

Aussi, l'aire des zones de danger et de proximité sont nécessairement différentes, avec la zone de proximité bien plus étendues que la zone de danger. Le nombre point GPS enregistrés pour chaque individu dans chaque zone a donc été divisé par surface total de chaque zone afin de prendre en compte la non répartition homogène des points GPS entre les deux zones uniquement due à leur surface respective.
